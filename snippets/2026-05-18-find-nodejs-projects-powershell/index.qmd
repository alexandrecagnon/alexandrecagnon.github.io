...
I realised I had no idea how many Node.js projects were sitting on my machine. This snippet walks a projects folder, finds every `package.json` that belongs to a real project, and prints the result as JSON. I have many Python projects so I added a parameter to include or exclude scanning under `.venv` packages.
...
**When `$IncludeVenv` is `$false`** - a log line appears before the JSON: `Packages under .venv (excluded from scan): N`.
...
**When `$IncludeVenv` is `$true`** - the JSON includes the `.venv` packages:
```json
{
  "rootPath": "C:\\projects",
  "includeVenv": true,
